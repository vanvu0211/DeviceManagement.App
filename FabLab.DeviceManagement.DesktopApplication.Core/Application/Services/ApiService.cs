...
namespace FabLab.DeviceManagement.DesktopApplication.Core.Application.Services
{
    public class ApiService : IApiService
    {
        ...
        private const string serverUrl = "http://fablab.runasp.net";
        ...
        public async Task<IEnumerable<EquipmentDto>> GetAllEquipmentsAsync()
        {
            ...
            HttpResponseMessage response = await _httpClient.GetAsync($"{serverUrl}/api/Equipment?pageSize=1000&pageNumber=1");
        ...
        {
            HttpResponseMessage response = await _httpClient.GetAsync($"{serverUrl}/api/Equipment/Enhanced?pageSize=1000&pageNumber=1&Status=0");
        ...
        public async Task<IEnumerable<EquipmentDto>> GetEquipmentsRecordsAsync(string equipmentId, string equipmentName, string yearOfSupply, string equipmentTypeId, string? category, string? status, string[] Tags)
        {
            ...
            HttpResponseMessage response = await _httpClient.GetAsync($"{serverUrl}/api/Equipment/Enhanced?pageSize=1000&pageNumber=1" + Status + TagIds + Category + EquipmentTypeId + EquipmentId + EquipmentName);
        ...
        {
            HttpResponseMessage response = await _httpClient.GetAsync($"{serverUrl}/api/Equipment?search={search}&pageSize=1000&pageNumber=1");
...
#pragma warning restore CA2200 // Rethrow to preserve stack details
        ...
        //Note
        public async Task<IEnumerable<NoteDto>> GetNoteOfEquipment(string equipmentId)
        {
            HttpResponseMessage response = await _httpClient.GetAsync($"{serverUrl}/api/Note?equimentId={equipmentId}");
            response.EnsureSuccessStatusCode();
            string responseBody = await response.Content.ReadAsStringAsync();

            var responses = JsonConvert.DeserializeObject<IEnumerable<NoteDto>>(responseBody);
            if (responses is null)
            {
        ...
        public async Task CreateNote(NoteDto createNote)
        {
            ...
            HttpResponseMessage response = await _httpClient.PostAsync($"{serverUrl}/api/Note", content);
...
#pragma warning restore CA2200 // Rethrow to preserve stack details
        ...
        public async Task FixNote(NoteDto fixNote)
        {
            var json = JsonConvert.SerializeObject(fixNote);
            var content = new StringContent(json, Encoding.UTF8, "application/json");

            HttpResponseMessage response = await _httpClient.PutAsync($"{serverUrl}/api/Note", content);
            response.EnsureSuccessStatusCode();
        }
    }
}
