...
namespace FabLab.DeviceManagement.DesktopApplication.Core.Domain.Dtos.Equipments
{
    public class EquipmentDto: BaseViewModel
    {
        ...
        public string Note
        {
            ...
            {
                _Note = value;
                if(Note != "") NoteStatus = true;
                else NoteStatus = false;
            }
        }

        

        public EquipmentDto(string equipmentId, string equipmentName, DateTime yearOfSupply, string codeOfManager, string locationId, string supplierName, EStatus status, string equipmentTypeId, bool isChecked, string note)
        {
